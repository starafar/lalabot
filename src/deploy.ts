...
const commands: RESTPostAPIChatInputApplicationCommandsJSONBody[] =
  await Promise.all(
    globbySync("./commands/*.js", { cwd: "dist" }).map(
      async (commandFile: string) => {
...
const api: REST = new REST().setToken(process.env.DISCORD_API_TOKEN);

if (process.env.DEVELOPMENT_MODE) {
  log.info(
    "Currently running in development mode. Registering commands as guild commands..."
  );
  await api.put(
